...
namespace Shared.DependencyInjector.Internal
{
    public static class ExtensionMethods
    {
        ...
        public static TSource OnlyOrDefault<TSource>(this IEnumerable<TSource> source) => source.Count() > 1 ? default : source.FirstOrDefault();
        ...
        internal static IEnumerable<T> AllAttributes<T>(this MemberInfo provider) where T : Attribute => provider.AllAttributes(typeof(T)).Cast<T>();
        ...
        static IEnumerable<Attribute> AllAttributes(this MemberInfo provider, params Type[] attributeTypes)
        {
            ...
            return attributeTypes.Length == 0 ? allAttributes : allAttributes.Where(a => attributeTypes.Any(x => a.GetType().DerivesFromOrEqual(x)));
        }
    }
}